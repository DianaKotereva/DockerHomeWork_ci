...
class TelegramBot:
    ...
    def buttons(self, message, params):
        ...
        msg = bot.send_message(
            message.from_user.id, text=question, reply_markup=markup)
    ...
    def make_res(self, message, params):
        ...
        if message.text == 'Какие модели обучены?':
            ...
            params['to_do'] = 'get'
            bot.send_message(message.from_user.id, 'Нужен номер модели')
    ...
    def train_sel(self, message, params):
        ...
        if message.text == 'Новая модель':
            ...
            bot.send_message(
                message.from_user.id, 'Какую модель? Нужен номер изменяемой модели')
            self.select_one(message, params)
    ...
    def select_type(self, message, params):
        ...
        bot.send_message(message.from_user.id, 'Проверим train данные')
        if params['train'] is None or params['y_train'] is None:
            bot.send_message(
                message.from_user.id, 'Train данных нет. Пожалуйста, пришлите train датасет (без таргета) в формате json:')
    ...
    def check_yes_no(self, message, params):
        ...
        if params['step'] == 'Обучить модель':
            if message.text == 'Да':
                bot.send_message(
                    message.from_user.id, 'Хорошо. Пожалуйста, пришлите train датасет (без таргета) в формате json')
                bot.register_next_step_handler(message, self.get_train_data, params)
            ...
            if message.text == 'Да':
                bot.send_message(message.from_user.id, 'Хорошо. Введите cv')
            ...
            if message.text == 'Да':
                bot.send_message(
                    message.from_user.id, 'Хорошо. Пожалуйста, введите строку с config')
                bot.register_next_step_handler(message, self.get_config, params)
            else:
                bot.send_message(
                    message.from_user.id, 'Хорошо. Начинаю обучать модель')
                self.train_model(message, params)

        elif params['step'] == 'Получить тестовые данные':
            if message.text == 'Да':
                bot.send_message(
                    message.from_user.id, 'Хорошо. Пожалуйста, пришлите y_test array в формате json')
                bot.register_next_step_handler(message, self.get_y_test_data, params)
            else:
                params['y_test'] = None
                bot.send_message(message.from_user.id, 'Хорошо. Начинаю прогноз')
    ...
    def get_cv(self, message, params):
        ...
        try:
            ...
            bot.send_message(message.from_user.id, 'Хорошо. Введите n_trials')
    ...
    def get_config(self, message, params):
        ...
        bot.send_message(message.from_user.id, 'Хорошо. Начинаю обучать модель')
        self.train_model(message, params)
...
#             params['train'] = pd.DataFrame(json.loads(params['train']))
        ...
        bot.send_message(
            message.from_user.id, 'Хорошо. Пожалуйста, пришлите y_train array в формате json')
    ...
    def get_y_train_data(self, message, params):
        ...
        try:
            ...
            if len(np.unique(params['y_train'])) > 2:
                bot.send_message(
                    message.from_user.id, 'Класс обучает БИНАРНЫЕ классификации! Пришлите таргет еще раз')
                bot.register_next_step_handler(message, self.get_y_train_data, params)
            else:
                params['step'] = 'Подобрать гиперпараметры'
...
#             params['test'] = pd.DataFrame(json.loads(params['test']))
    ...
    def get_y_test_data(self, message, params):
        ...
        try:
            ...
            if len(np.unique(params['y_test'])) > 2:
                ...
                bot.send_message(message.from_user.id, 'Начинаю прогноз')
        ...
        try:
            ...
            if type(res) == list:
                ...
                if 'message' in list(res.keys()):
                    bot.send_message(
                        message.from_user.id, f"Ошибка: {res['message']}")
                else:
                    with open('predict.txt', 'w') as file:
                    ...
                    bot.send_document(message.from_user.id, f)
                    bot.send_message(
                        message.from_user.id, f"Модель {params['number']}. \n Accuracy: {res['acc']}. \n ROC_AUC: {res['roc_auc']}. \n APS: {res['aps']}")
    ...
    def train_model(self, message, params):
        ...
        try:
            ...
            if 'message' in list(res.keys()):
                bot.send_message(
                    message.from_user.id, f"Модель не обучена, ошибка: {res['message']}")
                ...
                bot.send_message(
                    message.from_user.id, f"Модель обучена\n Accuracy: {res['acc']}. \n ROC_AUC: {res['roc_auc']}. \n APS: {res['aps']}")
            self.buttons(message, params)
        except Exception as e:
            bot.send_message(message.from_user.id, f"Error: {e}")
            self.buttons(message, params)
    ...
    def delete(self, message, params):
        ...
        if message.text == 'Все':
            ...
            bot.send_message(
                message.from_user.id, 'Все модели удалены, возвращаюсь на стартовое меню')
            self.buttons(message, params)

        elif message.text == 'Одну модель':
            bot.send_message(
                message.from_user.id, 'Какую модель? Нужен номер удаляемой модели')
            self.select_one(message, params)
    ...
    def select_one(self, message, params):
        ...
        if res == 0:
            bot.send_message(
                message.from_user.id, 'Простите, но пока не обучено ни одной модели.')
            self.buttons(message, params)
        else:
            btts = []
            for n in range(res):
    ...
    def process_one(self, message, params):
        ...
        if params['to_do'] == 'delete':
            ...
            bot.send_message(message.from_user.id, f'Модель {number} удалена')
            self.buttons(message, params)
            ...
            bot.send_message(
                message.from_user.id, f"Модель {number}. Тип: {self.model_classes[r['name']]}. \n Accuracy: {r['acc']}. \n ROC_AUC: {r['roc_auc']}. \n APS: {r['aps']}")
            self.buttons(message, params)

        elif params['to_do'] == 'predict':
            bot.send_message(
                message.from_user.id, 'Загрузите test данные в формате json')
...
def start(message):
    ...
    if message.text == '/start':
        resp = requests.get("http://host.docker.internal:5000/api/check_user", data = {'user_id': message.from_user.id})
